...
TP_ROI_1 = float(os.getenv("TP_ROI_1", "50"))                 # target ROI% untuk TP1
TP_ROI_2 = float(os.getenv("TP_ROI_2", "100"))                 # target ROI% untuk TP2
TP_ROI_3 = float(os.getenv("TP_ROI_3", "150"))                 # target ROI% untuk TP3
...
class SignalEngine:
    ...
    def _build_cornix_signal(self, st: SetupState, price: float, zname: str) -> str:
        ...
        lines.append(f"1) {self._fmt(tp1)}")
        lines.append(f"2) {self._fmt(tp2)}")
        lines.append(f"3) {self._fmt(tp3)}")
    ...
    def process_symbol_tf(self, symbol: str, tf: str, price: float) -> None:
        ...
        self._process_smc_state(st_buy, price, demand, highs, lows, closes, curr_idx)
        self._process_smc_state(st_sell, price, supply, highs, lows, closes, curr_idx)

    def _process_smc_state(self, st: SetupState, price: float, active_zone: Optional[Zone], highs: List[float], lows: List[float], closes: List[float], curr_idx: int) -> None:
        if st.state == "SEARCHING":
            if not active_zone: return
            if st.side == "BUY" and price <= active_zone.high + st.buffer:
                st.state = "TAPPED_DEMAND"
            ...
            if closes[curr_idx] > st.choch_level:
                st.state = "CONFIRMED_BUY"
                # Use confirmation close price as actual entry
                entry = closes[curr_idx]
                emin = min(entry, st.choch_level)
                emax = max(entry, st.choch_level)
                sl = st.extreme_price - (st.atr * 0.2)
                
                # Failsafe: SL must be strictly below Entry for BUY
                if sl >= entry:
                    sl = entry - max(st.atr * 1.0, entry * 0.005)
                ...
                st.levels = {"entry": entry, "entry_min": emin, "entry_max": emax, "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3}
                self._send_confirmed(st, price, "DEMAND")
            elif price < active_zone.low - (st.atr * 1.0):
                st.reset_cycle()

        elif st.state == "TAPPED_SUPPLY":
            if not active_zone: 
                st.reset_cycle()
                return
            ...
            if closes[curr_idx] < st.choch_level:
                ...
                # Failsafe: SL must be strictly above Entry for SELL
                if sl <= entry:
                    sl = entry + max(st.atr * 1.0, entry * 0.005)
                ...
                st.levels = {"entry": entry, "entry_min": emin, "entry_max": emax, "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3}
                self._send_confirmed(st, price, "SUPPLY")
            elif price > active_zone.high + (st.atr * 1.0):
                st.reset_cycle()

        elif st.state in ("CONFIRMED_BUY", "CONFIRMED_SELL"):
